...
async function status(){try{const value=await api('/api/status');shareRoot=value.unc_prefix;$('product-version').textContent=value.version?'v'+value.version:'';$('status').textContent='已连接 NAS';$('index-status').textContent=`${Number(value.entries||0).toLocaleString()} 个索引条目 · ${value.scanning?'正在更新':value.dirty?'有待更新变化':'索引已就绪'}`;if(!connected){connected=true;explorer.setConnected(true);$('login').close();explorer.search();}}catch(e){if(connected)toast(e.message);}}
