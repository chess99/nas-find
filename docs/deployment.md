...
| 网络 | 监听地址、端口、允许访问的网段 |
| 文件访问 | UNC 共享根路径，以及 Windows 上可选的映射盘 |
...
通过网页登录验证服务；若只监听回环地址，可先使用 SSH 本地转发测试。Windows 客户端的构建安装见[开发文档](development.md)。首次启动时服务和共享地址为空，需填写自己的连接信息；已有用户保存的地址和映射盘不会被新默认值覆盖。
