...
let toastTimer,connected=false,poll;
...
const explorer=new Explorer($('explorer'),{
  ...
  action:async(file,action)=>{
    ...
    if(action==='copy_file')toast('文件已复制，可在资源管理器粘贴');
...
async function connect(event){
  ...
  try{const result=await invoke('connect',{config:{server:$('server').value,share:$('share').value,drive:$('drive').value,prefer_drive:$('prefer-drive').checked},password:$('password').value,remember:$('remember').checked});
    ...
    $('path-mode').textContent=result.config.prefer_drive&&result.mapping?.toLowerCase()===result.config.share.toLowerCase()?`打开位置 ${result.config.drive}\\`:'打开位置 UNC 共享';
...
async function start(){try{const value=await invoke('bootstrap');const c=value.config;$('server').value=c.server;$('share').value=c.share;$('drive').value=c.drive;$('prefer-drive').checked=c.prefer_drive;$('remember').checked=value.remembered;if(value.remembered)await connect();else settings();}catch(e){toast(String(e));}}
start();
