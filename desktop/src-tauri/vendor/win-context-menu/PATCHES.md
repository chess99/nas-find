...
- `src/path_parser.rs` / `src/shell_item.rs`: normalize paths without opening each file, cache parent folders and their ancestors for one selection, and parse child names relative to those folders. Real PIDLs retain long names; the desktop-parser fallback handles roots and background menus. The invisible results view uses list mode and disables redraw.
- `src/selection_data.rs` / `src/context_menu.rs`: for long-path selections, use the native default context menu callback to supply the complete Unicode CF_HDROP list before handlers receive it. Other native data formats and command dispatch remain owned by the Shell. Callback failure prevents showing/invoking a partially initialized menu. This is generic compatibility with file-drop consumers, with no player-specific registry lookup or command line.
- Native tests validate both the Shell item array and the full CF_HDROP list, including long paths. Run them with `--test-threads=1` because installed third-party extensions may not tolerate concurrent menu enumeration.
