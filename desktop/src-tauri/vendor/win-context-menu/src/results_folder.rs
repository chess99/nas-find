...
use std::{path::Path, rc::Rc, time::{Duration, Instant}};
use windows::Win32::{Foundation::{E_ABORT, E_FAIL, RECT}, System::Com::{CoCreateInstance, CLSCTX_INPROC_SERVER},
    UI::{Shell::*, WindowsAndMessaging::*}};
use crate::{com::Pidl, error::{Error, Result}, hidden_window::HiddenWindow,
    shell_item::ShellItems, util::{path_to_wide, strip_extended_prefix, wide_to_pcwstr}};
...
impl Drop for ResultsHost {
    fn drop(&mut self) {
        ...
        unsafe { let _ = self.browser.Destroy(); }
    }
}
...
pub(crate) fn from_paths(paths: &[impl AsRef<Path>]) -> Result<ShellItems> {
    ...
    for path in paths {
        ...
        let canonical = std::fs::canonicalize(path).map(|p|strip_extended_prefix(&p)).unwrap_or_else(|_|path.to_path_buf());
        let wide = path_to_wide(&canonical);
        let mut pidl = std::ptr::null_mut();
        // SHParseDisplayName allocates a PIDL; Pidl assumes its ownership.
        unsafe {
            SHParseDisplayName(wide_to_pcwstr(&wide), None, &mut pidl, 0, None)
                .map_err(|source| Error::ParsePath {path:canonical,source})?;
            absolute.push(Pidl::from_raw(pidl));
        }
    }
    ...
    unsafe {
        ...
        let host=Rc::new(ResultsHost {browser,_window:window});
        host.browser.SetOptions(EBO_NOPERSISTVIEWSTATE|EBO_NOTRAVELLOG|EBO_NOBORDER|EBO_NAVIGATEONCE)?;
        let settings=FOLDERSETTINGS {ViewMode:FVM_DETAILS.0 as u32,
            fFlags:(FWF_NOICONS|FWF_NOGROUPING|FWF_NOBROWSERVIEWSTATE).0 as u32};
        host.browser.Initialize(host._window.hwnd,&RECT {left:0,top:0,right:1,bottom:1},Some(&settings))?;
        ...
        for absolute in &references {
            ...
            if child.is_null() {return Err(Error::Windows(windows::core::Error::from_hresult(E_FAIL)));}
            children.push(Pidl::from_raw(child));
        }
        Ok(ShellItems {parent,child_pidls:children,_absolute_pidls:absolute,is_background:false,results_host:Some(host)})
    }
}
...
mod tests {
    ...
    use windows::Win32::{System::{Com::{IDataObject, FORMATETC}, Ole::ReleaseStgMedium}, Foundation::HWND};
    ...
    fn verify(paths: &[std::path::PathBuf]) {
        ...
        if paths.windows(2).any(|p|p[0].parent()!=p[1].parent()) {
            assert!(items.results_host.is_some(), "expected a cross-directory selection");
        }
        ...
        unsafe {
            let object:IDataObject=items.parent.GetUIObjectOf(HWND::default(),&ids,None).unwrap();
            let mut medium=object.GetData(&FORMATETC {cfFormat:15,dwAspect:1,lindex:-1,tymed:1,..Default::default()}).unwrap();
            ...
            for i in 0..count {
                ...
                actual.push(std::path::PathBuf::from(String::from_utf16_lossy(&buffer[..length as usize])));
            }
            ReleaseStgMedium(&mut medium);
            actual.sort();let mut expected=paths.to_vec();expected.sort();
            assert_eq!(actual,expected,"the Shell must receive every selected path exactly once");
        }
        let menu=ContextMenu::new(items).unwrap();
        let labels=menu.enumerate().unwrap();
        assert!(labels.iter().any(|item|item.command_string.as_deref()==Some("copy")));
    }
    ...
    fn cross_directory_data_objects_preserve_files_types_and_folders() {
        ...
        let left=temp.path().join("left");let right=temp.path().join("right");
        std::fs::create_dir(&left).unwrap();std::fs::create_dir(&right).unwrap();
        let first=left.join("同名 视频.mp4");let second=right.join("同名 视频.mp4");let text=right.join("notes.txt");
        for path in [&first,&second,&text] {std::fs::write(path,b"fixture").unwrap();}
        let folder=right.join("folder");std::fs::create_dir(&folder).unwrap();std::fs::write(folder.join("not-selected.txt"),b"do not enumerate").unwrap();
        verify(&[first.clone(),second]);
        verify(&[first.clone(),text]);
        verify(&[first,folder]);
        verify(&[left,right]);
        assert!(ShellItems::from_paths(&[temp.path().join("missing/a.txt"),temp.path().join("missing/b.mp4")]).is_err());
    }
}
